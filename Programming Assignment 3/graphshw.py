...
class WeightedGraph(Graph) :
    ...
    def _dijkstra(self, s, pq) :
        ...
        # Note to professor: This is a modified version of the inner-class that you implemented
        # in your dfs method. I thought it would be a good idea to use this to keep track of the
        # d-values and pi-values.
        class VertexData :
            __slots__ = [ 'd', 'pi' ]
        ...
        # This is the set of all the 3-tuples which will be returned at the end
        # of this method
        completed_vertices = []

        # These 2 lines essentially perform the Init-Single-Source(G,s)
        # method from the pseudocode of Dijkstra's algorithm
        vertexes = [VertexData() for i in range(self.num_vertexes())]
        vertexes[s].d = 0

        # Here, I add all the vertices into the priority queue, where the
        # priority is their shortest weight path (d-value)
        for i,u in enumerate(vertexes):
            pq.insert(i, u.d)


        # This is where Dijkstra's algorithm begins, and we go through the
        # entire priority queue
        while not pq.is_empty():

            # This is where we extract the element with the lowest priority
            # (lowest d-value), and also add it to the set of completed_vertices
            # which will be returned at the very end of the method
            curr_vertex = pq.extract_min()
            completed_vertices.append((curr_vertex,vertexes[curr_vertex].d,vertexes[curr_vertex].pi))

            # This is where we traverse each vertex in the adjacency list of the
            # current_vertex, and perform "Relax"
            for v, w in self._adj[curr_vertex].__iter__(True):
                curr_distance = vertexes[v].d

                # Here is the "Relax" method, which will be used to
                # update the shortest path from the source "s"
                if vertexes[v].d > vertexes[curr_vertex].d + w:
                    vertexes[v].d = vertexes[curr_vertex].d + w
                    vertexes[v].pi = curr_vertex

                # This if-statement will check to see the d-value of the
                # curr_vertex has changed, and if it has, then we will
                # perform the "Decrease-Key" method, which is just the
                # change_priority() method
                if pq.contains(v) and vertexes[v].d != curr_distance:
                    pq.change_priority(v, vertexes[v].d)

        # Lastly, we return the set of 3-tuples of all the vertices
        return completed_vertices
